...
def main():
	...
	if not partest_1_bool or not partest_2_bool or not partest_3_bool:
		...
		node.setAttribute("name", "sum")
		node.setAttribute("value", str(partest_1+partest_2+partest_3))
    	...
    	# metric3
		node=doc.createElement("system_metric")
		node.setAttribute("name", "difference")
		node.setAttribute("value", str(partest_1-partest_2-partest_3))
		root.appendChild(node)
        #print "------> simple_sim is running: Received "+str(partest_1)+" "+str(partest_2)+" "+str(partest_3)+" "
