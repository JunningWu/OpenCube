...
#define YACC_CREATE_TYPE(a) make_data_type(a)
#define YACC_RETURN_TOKEN(a) make_return_token(a)

#define YACC_PRINT_BUFFER() print_buffer()

void print_buffer(char ch) {
	printf("%c - %s\n", ch, buffer);
}

CxxBuiltInId *make_data_type(CxxToken* type) {
	return (CxxBuiltInId*)type;
}

CxxExpression *make_return_token(CxxToken* token) {
	if (strncmp(last_token, "return", 6) == 0) {
		put_end_function(1);
	}
	return (CxxExpression*)token;
}
...
CxxExpression *make_add_expression(CxxExpression *leftExpr, CxxExpression *rightExpr) {
	/*cout << "leftExpr: " << leftExpr << ", rightExpr: " << rightExpr << endl;*/
	return 0;
}
...
CxxBuiltInId *make_built_in_ids(CxxBuiltInId *anExpr, CxxBuiltInId *anElement) {
	if( anExpr->value()==UNSIGNED ) { /*delete anExpr;*/ return anElement; }
	if( anExpr->value()==SIGNED ) { /*delete anExpr;*/ return anElement; }
	if( anExpr->value()==SHORT ) { /*delete anExpr;*/ return anElement; }
	if( anElement->value()==LONG && anExpr->value()==LONG) {
		anExpr->set_name("long long");
		anExpr->set_value(LONG_LONG);
//		delete anElement;
		return anExpr;
	}
	if( anExpr->value()==LONG ) {/*delete anExpr;*/ return anElement;}
	if(anElement->value()==UNSIGNED) {/*delete anElement;*/ return anExpr;}
	if(anElement->value()==SIGNED) {/*delete anElement;*/ return anExpr;}
	increment_error_count();
	cout << "Error detected: Data type identifiers not compatible "<< anExpr->name()<<" , " << anElement->name() << endl;
	return 0;
}
...
CxxName *make_destructor_id(CxxName *aName) {
	char *str = (char*)malloc(strlen(aName->name()+2));
	strcpy(str,"~");
	strcat(str,aName->name());
	aName->set_name(str);
	free(str);
	return aName;
	}
...
CxxName *make_global_id(CxxIsTemplate isTemplate, CxxName *nestedId) { if(nestedId!=NULL)nestedId->set_scope("::");return nestedId; }
...
CxxName *make_name(CxxName *aName) { return aName; }
...
CxxName *make_nested_id(CxxName *nestingId, CxxName *nestedId) {
	if(nestedId==NULL) return nestingId;
	using_scope_operator();
	nestedId->set_scope(nestingId->name());
//	delete nestingId;
	return nestedId;
}
CxxName *make_nested_scope(CxxName *nestingId) {
	int size=strlen(nestingId->name());
	char* str=(char*)malloc(size+4);
	strncpy(str,nestingId->name(),size+1);
	strncat(str,"::",3);
	nestingId->set_name(str);
	free(str);
	return nestingId;
}
...
CxxName *make_set_template_id(CxxName *aName) { return aName; }
...
CxxName *make_template_name(CxxName *aName, CxxTemplateArguments *templateArguments) { return aName; }
