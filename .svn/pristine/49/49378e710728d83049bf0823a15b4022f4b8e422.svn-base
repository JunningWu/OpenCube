...
%term <built_in_id> BOOL CHAR DOUBLE FLOAT INT LONG SHORT SIGNED UNSIGNED VOID WCHAR_T LONG_LONG
...
%type <expression> if_keyword return_token
%type <statement>  forced_looping_statement init_compound_statement forced_looped_statement
...
identifier:
		identifier_word										%prec SHIFT_THERE { mark_identifier(); }
/*
 ...
 */
id:
		identifier %prec SHIFT_THERE 									{ $$ = YACC_NAME($1); } /* Force < through test */
	|	identifier template_test '+' template_argument_list '>'			{ $$ = YACC_TEMPLATE_NAME($1, $4); }
    |	identifier template_test '+' '>'								{ $$ = $1; WRNMSG("Empty template-argument-list"); }
    |	identifier template_test '-'									/* requeued < follows */  { $$ = YACC_NAME($1); }
    |	template_id

template_test:
		'<'																/* Queue '+' or '-' < as follow on */ { template_test(); }
 ...
 */
nested_id:
		id																%prec SHIFT_THERE /* Maximise length */ { $$ = $1; }
...
pseudo_destructor_id:
		built_in_type_id SCOPE '~' built_in_type_id						{ $$ = YACC_PSEUDO_DESTRUCTOR_ID($1, $4); }
 ...
 */
string:
		StringLiteral													{ mark_literal(); $$ = $1; }
/*string:
		StringLiteral													%prec SHIFT_THERE { $$ = YACC_STRINGS($1, 0); }
	|	StringLiteral string  -- Perverse order avoids conflicts -- 	{ $$ = YACC_STRINGS($1, $2); } */

literal:
		IntegerLiteral													{ mark_literal(); $$ = YACC_INTEGER_LITERAL_EXPRESSION($1); }
	|	CharacterLiteral												{ mark_literal(); $$ = YACC_CHARACTER_LITERAL_EXPRESSION($1); }
	|	FloatingLiteral													{ mark_literal(); $$ = YACC_FLOATING_LITERAL_EXPRESSION($1); }
	|	string															{ mark_literal(); $$ = YACC_STRING_LITERAL_EXPRESSION($1); }
	|	boolean_literal

boolean_literal:
		FALSE															{ mark_literal(); $$ = YACC_FALSE_EXPRESSION(); }
	|	TRUE															{ mark_literal(); $$ = YACC_TRUE_EXPRESSION(); }

/*---------------------------------------------------------------------------------------------------
 * A.3 Basic concepts
 *---------------------------------------------------------------------------------------------------*/
translation_unit:
		declaration_seq.opt												{ YACC_RESULT($1); }
 ...
 */
abstract_expression:
			parenthesis_clause												{ $$ = YACC_ABSTRACT_FUNCTION_EXPRESSION($1); }
	|		'[' expression.opt ']'											{ /*add_operator_cost(ARRAY_OP);*/ $$ = YACC_ABSTRACT_ARRAY_EXPRESSION($2); }
 ...
 */
type1_parameters:
		parameter_declaration_list ';'										{ $$ = YACC_TYPE1_PARAMETERS(0, $1); }
	|	type1_parameters parameter_declaration_list ';'						{ $$ = YACC_TYPE1_PARAMETERS($1, $2); }

mark_type1:																	{ $$ = mark_type1(); }

postfix_expression:
		primary_expression
/*  |	postfix_expression parenthesis_clause								{ $$ = YACC_CALL_EXPRESSION($1, $2); } */
	|	postfix_expression parenthesis_clause mark_type1 '-'				{ function_decl(); $$ = YACC_CALL_EXPRESSION($1, $2); }
	|	postfix_expression parenthesis_clause mark_type1 '+' type1_parameters mark '{' error
				{ yyerrok; remark_type1($6); unmark(); unmark($5); function_decl();
				  $$ = YACC_TYPE1_EXPRESSION($1, $2, $5); }
	|	postfix_expression parenthesis_clause mark_type1 '+' type1_parameters mark error
																			{ yyerrok; remark_type1($3); unmark(); unmark(); function_decl();
																			  $$ = YACC_CALL_EXPRESSION($1, $2); }
	|	postfix_expression parenthesis_clause mark_type1 '+' error
																			{ yyerrok; remark_type1($3); unmark(); function_decl();
																			  $$ = YACC_CALL_EXPRESSION($1, $2); }
	|	postfix_expression '[' expression.opt ']'							{ add_operator_cost(ARRAY_OP); $$ = YACC_ARRAY_EXPRESSION($1, $3); }
...
/*  |	postfix_expression '.' TEMPLATE declarator_id						-- TEMPLATE absorbed into declarator_id. */
	...
	|	postfix_expression ARROW declarator_id								{ add_operator_cost(ARROW_OP); $$ = YACC_ARROW_EXPRESSION($1, $3); }
/*  |	postfix_expression ARROW TEMPLATE declarator_id						-- TEMPLATE absorbed into declarator_id. */
	|	postfix_expression ARROW scoped_pseudo_destructor_id				{ add_operator_cost(ARROW_OP); $$ = YACC_ARROW_EXPRESSION($1, $3); }
	|	postfix_expression INC												{ add_operator_cost(INC_OP); $$ = YACC_POST_INCREMENT_EXPRESSION($1); }
	|	postfix_expression DEC												{ add_operator_cost(DEC_OP); $$ = YACC_POST_DECREMENT_EXPRESSION($1); }
...
expression_list.opt:														{ $$ = YACC_EXPRESSIONS(0, 0); }
	|	expression_list

expression_list:
		assignment_expression												{ $$ = YACC_EXPRESSIONS(0, $1); }
	|	expression_list ',' assignment_expression							{ $$ = YACC_EXPRESSIONS($1, $3); }

unary_expression:
		postfix_expression
	|	INC cast_expression													{ add_operator_cost(INC_OP); $$ = YACC_PRE_INCREMENT_EXPRESSION($2); }
	|	DEC cast_expression													{ add_operator_cost(DEC_OP); $$ = YACC_PRE_DECREMENT_EXPRESSION($2); }
	|	ptr_operator cast_expression										{ add_operator_cost(STAR_OP); $$ = YACC_POINTER_EXPRESSION($1, $2); }
...
/*  |	decl_specifier_seq '&' cast_expression								-- covered by binary operator */
	...
	|	'+' cast_expression													{ add_operator_cost(PLUS_OP); $$ = YACC_PLUS_EXPRESSION($2); }
	|	'-' cast_expression													{ add_operator_cost(MINUS_OP); $$ = YACC_MINUS_EXPRESSION($2); }
	|	'!' cast_expression													{ add_operator_cost(NOT_OP); $$ = YACC_NOT_EXPRESSION($2); }
	|	'~' cast_expression													{ add_operator_cost(INV_OP); $$ = YACC_COMPLEMENT_EXPRESSION($2); }
...
delete_expression:
		DELETE cast_expression												/* also covers DELETE[] cast_expression */ { $$ = YACC_DELETE_EXPRESSION($2); }

new_expression:
		NEW new_type_id new_initializer.opt									{ $$ = YACC_NEW_TYPE_ID_EXPRESSION(0, $2, $3); }
...
new_type_id:
		type_specifier ptr_operator_seq.opt									{ $$ = YACC_TYPED_EXPRESSION($1, $2); }
	|	type_specifier new_declarator										{ $$ = YACC_TYPED_EXPRESSION($1, $2); }
	|	type_specifier new_type_id											{ $$ = YACC_TYPED_EXPRESSION($1, $2); }

new_declarator:
		ptr_operator new_declarator											{ $$ = YACC_POINTER_EXPRESSION($1, $2); }
	|	direct_new_declarator

direct_new_declarator:
		'[' expression ']'													{ $$ = YACC_ABSTRACT_ARRAY_EXPRESSION($2); }
	|	direct_new_declarator '[' constant_expression ']'					{ $$ = YACC_ARRAY_EXPRESSION($1, $3); }

new_initializer.opt:														{ $$ = YACC_EXPRESSIONS(0, 0); }
...
multiplicative_expression:
	...
	|	multiplicative_expression '/' pm_expression							{ add_operator_cost(DIV_OP); $$ = YACC_DIVIDE_EXPRESSION($1, $3); }
	|	multiplicative_expression '%' pm_expression							{ add_operator_cost(MOD_OP); $$ = YACC_MODULUS_EXPRESSION($1, $3); }

additive_expression:
		multiplicative_expression
	|	additive_expression '+' multiplicative_expression					{ add_operator_cost(PLUS_OP); $$ = YACC_ADD_EXPRESSION($1, $3); }
	|	additive_expression '-' multiplicative_expression					{ add_operator_cost(MINUS_OP); $$ = YACC_SUBTRACT_EXPRESSION($1, $3); }

shift_expression:
		additive_expression
	|	shift_expression SHL additive_expression							{ add_operator_cost(SHL_OP); $$ = YACC_SHIFT_LEFT_EXPRESSION($1, $3); }
	|	shift_expression SHR additive_expression							{ add_operator_cost(SHR_OP); $$ = YACC_SHIFT_RIGHT_EXPRESSION($1, $3); }

relational_expression:
		shift_expression
	|	relational_expression '<' shift_expression							{ add_operator_cost(LT_OP); $$ = YACC_LESS_THAN_EXPRESSION($1, $3); }
	|	relational_expression '>' shift_expression							{ add_operator_cost(GT_OP); $$ = YACC_GREATER_THAN_EXPRESSION($1, $3); }
	|	relational_expression LE shift_expression							{ add_operator_cost(LE_OP); $$ = YACC_LESS_EQUAL_EXPRESSION($1, $3); }
	|	relational_expression GE shift_expression							{ add_operator_cost(GE_OP); $$ = YACC_GREATER_EQUAL_EXPRESSION($1, $3); }

equality_expression:
		relational_expression
	|	equality_expression EQ relational_expression						{ add_operator_cost(EQUAL_OP); $$ = YACC_EQUAL_EXPRESSION($1, $3); }
	|	equality_expression NE relational_expression						{ add_operator_cost(NEQ_OP); $$ = YACC_NOT_EQUAL_EXPRESSION($1, $3); }

and_expression:
		equality_expression
	|	and_expression '&' equality_expression								{ add_operator_cost(AND_OP); $$ = YACC_AND_EXPRESSION($1, $3); }

exclusive_or_expression:
		and_expression
	|	exclusive_or_expression '^' and_expression							{ add_operator_cost(XOR_OP); $$ = YACC_EXCLUSIVE_OR_EXPRESSION($1, $3); }

inclusive_or_expression:
		exclusive_or_expression
	|	inclusive_or_expression '|' exclusive_or_expression					{ add_operator_cost(OR_OP); $$ = YACC_INCLUSIVE_OR_EXPRESSION($1, $3); }

logical_and_expression:
		inclusive_or_expression
	|	logical_and_expression LOG_AND inclusive_or_expression				{ add_operator_cost(LOG_AND_OP); $$ = YACC_LOGICAL_AND_EXPRESSION($1, $3); }

logical_or_expression:
		logical_and_expression
	|	logical_or_expression LOG_OR logical_and_expression					{ add_operator_cost(LOG_OR_OP); $$ = YACC_LOGICAL_OR_EXPRESSION($1, $3); }

conditional_expression:
		logical_or_expression
	|	logical_or_expression '?' expression ':' assignment_expression		{ add_operator_cost(TERNARY_OP); $$ = YACC_CONDITIONAL_EXPRESSION($1, $3, $5); }
...
assignment_expression:
	...
	|	logical_or_expression '=' braced_initializer						{ add_operator_cost(ASSIGN_OP); $$ = YACC_ASSIGNMENT_EXPRESSION($1, $2, $3); }
	|	throw_expression

assignment_operator:
		'='																	{ add_operator_cost(ASSIGN_OP);}
	|	ASS_ADD																{ add_operator_cost(ASS_ADD_OP);}
	|	ASS_AND																{ add_operator_cost(ASS_AND_OP);}
	|	ASS_DIV																{ add_operator_cost(ASS_DIV_OP);}
	|	ASS_MOD																{ add_operator_cost(ASS_MOD_OP);}
	|	ASS_MUL																{ add_operator_cost(ASS_MUL_OP);}
	|	ASS_OR																{ add_operator_cost(ASS_OR_OP);}
	|	ASS_SHL																{ add_operator_cost(ASS_SHL_OP);}
	|	ASS_SHR																{ add_operator_cost(ASS_SHR_OP);}
	|	ASS_SUB																{ add_operator_cost(ASS_SUB_OP);}
	|	ASS_XOR																{ add_operator_cost(ASS_XOR_OP);}
...
expression.opt:																{ $$ = YACC_EXPRESSION(0); }
	|	expression
...
templated_relational_expression:
		shift_expression
	|	templated_relational_expression '<' shift_expression				{ add_operator_cost(LT_OP); $$ = YACC_LESS_THAN_EXPRESSION($1, $3); }
	|	templated_relational_expression LE shift_expression					{ add_operator_cost(LE_OP); $$ = YACC_LESS_EQUAL_EXPRESSION($1, $3); }
	|	templated_relational_expression GE shift_expression					{ add_operator_cost(GE_OP); $$ = YACC_GREATER_EQUAL_EXPRESSION($1, $3); }

templated_equality_expression:
		templated_relational_expression
	|	templated_equality_expression EQ templated_relational_expression	{ add_operator_cost(EQUAL_OP); $$ = YACC_EQUAL_EXPRESSION($1, $3); }
	|	templated_equality_expression NE templated_relational_expression	{ add_operator_cost(NEQ_OP); $$ = YACC_NOT_EQUAL_EXPRESSION($1, $3); }

templated_and_expression:
		templated_equality_expression
	|	templated_and_expression '&' templated_equality_expression			{ add_operator_cost(AND_OP); $$ = YACC_AND_EXPRESSION($1, $3); }

templated_exclusive_or_expression:
		templated_and_expression
	|	templated_exclusive_or_expression '^' templated_and_expression		{ add_operator_cost(XOR_OP); $$ = YACC_EXCLUSIVE_OR_EXPRESSION($1, $3); }

templated_inclusive_or_expression:
		templated_exclusive_or_expression
	|	templated_inclusive_or_expression '|' templated_exclusive_or_expression
																			{ add_operator_cost(OR_OP); $$ = YACC_INCLUSIVE_OR_EXPRESSION($1, $3); }

templated_logical_and_expression:
		templated_inclusive_or_expression
	|	templated_logical_and_expression LOG_AND templated_inclusive_or_expression
																			{ add_operator_cost(LOG_AND_OP); $$ = YACC_LOGICAL_AND_EXPRESSION($1, $3); }

templated_logical_or_expression:
		templated_logical_and_expression
	|	templated_logical_or_expression LOG_OR templated_logical_and_expression
																			{ add_operator_cost(LOG_OR_OP); $$ = YACC_LOGICAL_OR_EXPRESSION($1, $3); }

templated_conditional_expression:
		templated_logical_or_expression
	|	templated_logical_or_expression '?' templated_expression ':' templated_assignment_expression
																			{ add_operator_cost(TERNARY_OP);$$ = YACC_CONDITIONAL_EXPRESSION($1, $3, $5); }
...
templated_expression_list:
		templated_assignment_expression										{ $$ = YACC_EXPRESSIONS(0, $1); }
 ...
 */
looping_statement:
		start_search looped_statement										{ $$ = YACC_LINED_STATEMENT($2, $1); end_search($$); }
...
labeled_statement:
		identifier_word ':' looping_statement								{ $$ = YACC_LABEL_STATEMENT($1, $3); }
	|	CASE /*{new_block("case");}*/ constant_expression ':' looping_statement { $$ = YACC_CASE_STATEMENT($2, $4); }
	|	DEFAULT /*{ new_block("default"); }*/ ':' looping_statement				{ $$ = YACC_DEFAULT_STATEMENT($3); }

/*expression_statement:
		expression.opt ';'													-- covered by declaration_statement */

init_compound_statement:
		'{'																	{ start_compound_statement(); }

compound_statement:
		init_compound_statement '-' statement_seq.opt '}'					{ block_end(); $$ = YACC_COMPOUND_STATEMENT($3); }
	|	init_compound_statement '-' statement_seq.opt looping_statement '#' bang error '}'
																			{ /*block_end();*/ $$ = $2; YACC_UNBANG($6, "Bad statement-seq."); }
	|	init_compound_statement '+' statement_seq.opt '}'					{ $$ = YACC_COMPOUND_STATEMENT($3);  }
	|	init_compound_statement '+' statement_seq.opt looping_statement '#' bang error  '}'
																			{ $$ = $2; YACC_UNBANG($6, "Bad statement-seq."); }

statement_seq.opt:															{ $$ = YACC_STATEMENTS(0, 0); }
	|	statement_seq.opt looping_statement									{ $$ = YACC_STATEMENTS($1, YACC_COMPILE_STATEMENT($2)); }
	|	statement_seq.opt looping_statement '#' bang error ';'				{ end_statement(); $$ = $1; YACC_UNBANG($4, "Bad statement."); }
...
forced_looping_statement:
		start_search compound_statement										{ add_operator_cost(JUMP_ST); $$ = YACC_LINED_STATEMENT($2, $1);
																			  end_search($$); }
	|	{ put_block_start(); } start_search forced_looped_statement			{ $$ = YACC_LINED_STATEMENT($3, $2); end_search($$);
																			  put_block_end(); add_operator_cost(JUMP_ST); }

selection_statement:
		if_keyword '(' condition ')' forced_looping_statement				%prec SHIFT_THERE { $$ = YACC_IF_STATEMENT($3, $5, 0); }
	|	if_keyword '(' condition ')' forced_looping_statement ELSE  forced_looping_statement
																			{ $$ = YACC_IF_STATEMENT($3, $5, $7); }
	|	SWITCH { new_block("switch"); add_operator_cost(JUMP_ST); } '(' condition ')' forced_looping_statement
																			{ $$ = YACC_SWITCH_STATEMENT($4, $6); }

condition.opt: 																{ $$ = YACC_CONDITION(0); }
	|	condition

condition:
		parameter_declaration_list											{ $$ = YACC_CONDITION($1); }
...
iteration_statement:
		WHILE { new_block("while"); add_operator_cost(START_LOOP_ST);} '(' condition ')'  forced_looping_statement
																			{ $$ = YACC_WHILE_STATEMENT($4, $6); }
	|	DO { new_block("do"); add_operator_cost(START_LOOP_ST); } forced_looping_statement WHILE '(' expression ')' ';'
																			{ $$ = YACC_DO_WHILE_STATEMENT($3, $6); }
	|	FOR { new_block("for"); } '(' for_init_statement { save_init_for_cost(); add_operator_cost(START_LOOP_ST); }
		condition.opt ';' expression.opt { annotate_for(); } ')' forced_looping_statement { $$ = YACC_FOR_STATEMENT($4, $6, $8, $11); restore_init_for_cost(); }
...
return_token:
		RETURN																{ $$ = YACC_RETURN_TOKEN($1); }

jump_statement:
		BREAK { new_block("break");add_operator_cost(JUMP_ST);} ';' 		{ $$ = YACC_BREAK_STATEMENT(); }
	|	CONTINUE { new_block("continue");add_operator_cost(JUMP_ST);}';'	{ $$ = YACC_CONTINUE_STATEMENT(); }
	|	return_token expression.opt ';'										{ $$ = YACC_RETURN_STATEMENT($2); }
	|	GOTO { new_block("goto");add_operator_cost(JUMP_ST);} identifier ';'
																			{ $$ = YACC_GOTO_STATEMENT($3); }

declaration_statement:
		block_declaration													{ $$ = YACC_DECLARATION_STATEMENT($1); }

/*---------------------------------------------------------------------------------------------------
 * A.6 Declarations
 *---------------------------------------------------------------------------------------------------*/
compound_declaration:
		'{' nest declaration_seq.opt '}'									{ $$ = $3; unnest($2); }
	|	'{' nest declaration_seq.opt util looping_declaration '#' bang error '}'
																			{ $$ = $3; unnest($2); YACC_UNBANG($7, "Bad declaration-seq."); }

declaration_seq.opt:														{ $$ = YACC_DECLARATIONS(0, 0); }
	|	declaration_seq.opt util looping_declaration						{ $$ = YACC_DECLARATIONS($1, YACC_COMPILE_DECLARATION($2, $3)); }
	|	declaration_seq.opt util looping_declaration '#' bang error ';'		{ end_statement(); $$ = $1; YACC_UNBANG($5, "Bad declaration."); }

looping_declaration:
		start_search1 looped_declaration									{ $$ = YACC_LINED_DECLARATION($2, $1); end_search($$); }
...
simple_declaration:
		';'																	{ end_statement(); $$ = YACC_EXPRESSION(0); }
	|	init_declaration ';'												{ end_statement(); }
	|	init_declarations ';'												{ end_statement(); $$ = $1; }
...
suffix_built_in_decl_specifier:
		suffix_built_in_decl_specifier.raw									{ $$ = $1; }
...
suffix_named_decl_specifier.bi:
		suffix_named_decl_specifier											{ $$ = YACC_NAME_EXPRESSION($1); }
...
suffix_named_decl_specifiers.sf:
		scoped_special_function_id											{ $$ = YACC_NAME_EXPRESSION($1); } /* operators etc */
	|	suffix_named_decl_specifiers
...
built_in_type_specifier:
		CHAR																{ $$ = YACC_CREATE_TYPE($1); }
	|	WCHAR_T 															{ $$ = YACC_CREATE_TYPE($1); }
	|	BOOL 																{ $$ = YACC_CREATE_TYPE($1); }
	|	SHORT 																{ $$ = YACC_CREATE_TYPE($1); }
	|	INT 																{ $$ = YACC_CREATE_TYPE($1); }
	|	LONG 																{ $$ = YACC_CREATE_TYPE($1); }
	|	SIGNED 																{ $$ = YACC_CREATE_TYPE($1); }
	|	UNSIGNED 															{ $$ = YACC_CREATE_TYPE($1); }
	|	FLOAT 																{ $$ = YACC_CREATE_TYPE($1); }
	|	DOUBLE 																{ $$ = YACC_CREATE_TYPE($1); }
	|	VOID																{ $$ = YACC_CREATE_TYPE($1); }
...
elaborated_enum_specifier:
		ENUM scoped_id %prec SHIFT_THERE									{ $$ = YACC_ELABORATED_TYPE_SPECIFIER($1, $2); }

enum_specifier:
		ENUM scoped_id enumerator_clause									{ $$ = YACC_ENUM_SPECIFIER_ID($2, $3); }
	|	ENUM enumerator_clause												{ $$ = YACC_ENUM_SPECIFIER_ID(0, $2); }

enumerator_clause:
		'{' enumerator_list_ecarb											{ $$ = YACC_ENUMERATORS(0, 0); }
...
enumerator_list_head:
		enumerator_definition_filler										{ $$ = YACC_ENUMERATORS(0, 0); }
	|	enumerator_list ',' enumerator_definition_filler

enumerator_list:
		enumerator_list_head enumerator_definition							{ $$ = YACC_ENUMERATORS($1, $2); }

enumerator_definition:
		enumerator															{ $$ = YACC_ENUMERATOR($1, 0); }
...
namespace_definition:
		NAMESPACE scoped_id compound_declaration							{ $$ = YACC_NAMESPACE_DEFINITION($2, $3); }
	|	NAMESPACE compound_declaration										{ $$ = YACC_NAMESPACE_DEFINITION(0, $2); }

namespace_alias_definition:
		NAMESPACE scoped_id '=' scoped_id ';'								{ $$ = YACC_NAMESPACE_ALIAS_DEFINITION($2, $4); }

using_declaration:
		USING declarator_id ';'												{ $$ = YACC_USING_DECLARATION(false, $2); }
	|	USING TYPENAME declarator_id ';'									{ $$ = YACC_USING_DECLARATION(true, $3); }

using_directive:
		USING NAMESPACE scoped_id ';'										{ $$ = YACC_USING_DIRECTIVE($3); }

asm_definition:
		ASM '(' string ')' ';'												{ $$ = YACC_ASM_DEFINITION($3); }

linkage_specification:
		EXTERN string looping_declaration									{ $$ = YACC_LINKAGE_SPECIFIER($2, $3); }
...
init_declarations:
		assignment_expression ',' init_declaration							{ $$ = YACC_EXPRESSIONS(YACC_EXPRESSIONS(0, $1), $3); }
...
star_ptr_operator:
		'*'																	{ /*add_operator_cost(STAR_OP);*/ $$ = YACC_POINTER_DECLARATOR(); }
...
ptr_operator:
		'&'																	{ /*add_operator_cost(REFERENCE_OP);*/ $$ = YACC_REFERENCE_DECLARATOR(); }
	|	nested_ptr_operator													{ $$ = $1; }
	|	global_scope nested_ptr_operator									{ $$ = YACC_GLOBAL_DECLARATOR($1, $2); }

ptr_operator_seq:
		ptr_operator														{ $$ = YACC_POINTER_EXPRESSION($1, YACC_EPSILON()); }
...
ptr_operator_seq.opt:														/* empty */
		%prec SHIFT_THERE													{ $$ = YACC_EXPRESSION(0); } /* Maximise type length */
	|	ptr_operator ptr_operator_seq.opt									{ $$ = YACC_POINTER_EXPRESSION($1, $2); }

cv_qualifier_seq.opt:														{ $$ = YACC_CV_QUALIFIERS(0, 0); }
...
type_id:
		type_specifier abstract_declarator.opt								{ $$ = YACC_TYPED_EXPRESSION($1, $2); }
	|	type_specifier type_id												{ $$ = YACC_TYPED_EXPRESSION($1, $2); }

/*abstract_declarator:                                                      -- also covered by parameter declaration */
abstract_declarator.opt:													{ $$ = YACC_EPSILON(); }
	|	ptr_operator abstract_declarator.opt								{ $$ = YACC_POINTER_EXPRESSION($1, $2); }
	|	direct_abstract_declarator

direct_abstract_declarator.opt:												{ $$ = YACC_EPSILON(); }
	|	direct_abstract_declarator

direct_abstract_declarator:
		direct_abstract_declarator.opt parenthesis_clause					{ $$ = YACC_CALL_EXPRESSION($1, $2); }
...
parenthesis_clause:
		parameters_clause cv_qualifier_seq.opt								{ $$ = YACC_PARENTHESISED($1, $2, 0); }
...
parameter_declaration_clause:												{ $$ = YACC_PARAMETERS(0, 0); }
	|	parameter_declaration_list
	|	parameter_declaration_list ELLIPSIS									{ $$ = YACC_PARAMETERS($1, YACC_ELLIPSIS_EXPRESSION()); }

parameter_declaration_list:
		parameter_declaration												{ $$ = YACC_PARAMETERS(0, $1); }
...
special_parameter_declaration:
		abstract_parameter_declaration
	|	abstract_parameter_declaration '=' assignment_expression			{ add_operator_cost(ASSIGN_OP); $$ = YACC_ASSIGNMENT_EXPRESSION($1, $2, $3); }
	|	ELLIPSIS															{ $$ = YACC_ELLIPSIS_EXPRESSION(); }

parameter_declaration:
		assignment_expression												{ $$ = YACC_EXPRESSION_PARAMETER($1); }
...
templated_parameter_declaration:
		templated_assignment_expression										{ $$ = YACC_EXPRESSION_PARAMETER($1); }
...
func_definition:
		assignment_expression function_try_block							{ $$ = YACC_FUNCTION_DEFINITION($1, $2); }
...
ctor_definition:
		constructor_head function_try_block									{ $$ = YACC_CTOR_DEFINITION($1, $2); }
...
constructor_head:
		bit_field_init_declaration											{ $$ = YACC_EXPRESSIONS(0, $1); }
	|	constructor_head ',' assignment_expression							{ $$ = YACC_EXPRESSIONS($1, $3); }

function_try_block:
		TRY function_block handler_seq										{ $$ = YACC_TRY_FUNCTION_BLOCK($2, $3); }

function_block:
		ctor_initializer.opt function_body									{ $$ = YACC_CTOR_FUNCTION_BLOCK($2, $1); }

function_body:
		compound_statement													{ $$ = YACC_FUNCTION_BLOCK($1); put_end_function(); }
...
initializer_clause:
		assignment_expression												{ $$ = YACC_INITIALIZER_EXPRESSION_CLAUSE($1); }
	|	braced_initializer
...
initializer_list:
		looping_initializer_clause											{ $$ = YACC_INITIALIZER_CLAUSES(0, $1); }
	|	initializer_list ',' looping_initializer_clause						{ $$ = YACC_INITIALIZER_CLAUSES($1, $3); }

looping_initializer_clause:
		start_search looped_initializer_clause								{ $$ = $2; end_search($$); }
...
elaborated_class_specifier:
		class_key scoped_id %prec SHIFT_THERE								{ $$ = YACC_ELABORATED_TYPE_SPECIFIER($1, $2); }
	|	class_key scoped_id colon_mark error								{ $$ = YACC_ELABORATED_TYPE_SPECIFIER($1, $2); rewind_colon($3, $$); }

class_specifier_head:
		class_key scoped_id colon_mark base_specifier_list '{'				{ unmark($4); $$ = YACC_CLASS_SPECIFIER_ID($1, $2, $4); }
...
class_specifier:
		class_specifier_head member_specification.opt '}'					{ $$ = YACC_CLASS_MEMBERS($1, $2); }
	|	class_specifier_head member_specification.opt util looping_member_declaration '#' bang error '}'
																			{ $$ = YACC_CLASS_MEMBERS($1, $2);
																			YACC_UNBANG($6, "Bad member_specification.opt."); }

member_specification.opt:													{ $$ = YACC_MEMBER_DECLARATIONS(0, 0); }
...
looping_member_declaration:
		start_search looped_member_declaration								{ $$ = YACC_LINED_DECLARATION($2, $1); end_search($$); }
...
simple_member_declaration:
			';'																{ end_statement(); $$ = YACC_EXPRESSION(0); }
	|	assignment_expression ';'											{ end_statement(); }
	|	constructor_head ';'												{ end_statement(); $$ = $1; }
	|	member_init_declarations ';'										{ end_statement(); $$ = $1; }
	|	decl_specifier_prefix simple_member_declaration						{ $$ = YACC_DECL_SPECIFIER_EXPRESSION($2, $1); }

member_init_declarations:
		assignment_expression ',' member_init_declaration					{ $$ = YACC_EXPRESSIONS(YACC_EXPRESSIONS(0, $1), $3); }
...
accessibility_specifier:
		access_specifier ':'												{ $$ = YACC_ACCESSIBILITY_SPECIFIER($1); }

bit_field_declaration:
		assignment_expression ':' bit_field_width							{ $$ = YACC_BIT_FIELD_EXPRESSION($1, $3); }
...
/*  |	logical_or_expression '?' expression ':' assignment_expression		-- has SR conflict w.r.t later = */
	|	logical_or_expression '?' bit_field_width ':' bit_field_width		{ add_operator_cost(TERNARY_OP); $$ = YACC_CONDITIONAL_EXPRESSION($1, $3, $5); }

bit_field_init_declaration:
		bit_field_declaration
	|	bit_field_declaration '=' initializer_clause						{ add_operator_cost(ASSIGN_OP);$$ = YACC_ASSIGNMENT_EXPRESSION($1, $2, $3); }
...
base_specifier_list:
		base_specifier														{ $$ = YACC_BASE_SPECIFIERS(0, $1); }
	|	base_specifier_list ',' base_specifier								{ $$ = YACC_BASE_SPECIFIERS($1, $3); }

base_specifier:
		scoped_id															{ $$ = YACC_BASE_SPECIFIER($1); }
...
conversion_function_id:
		OPERATOR conversion_type_id											{ $$ = YACC_CONVERSION_FUNCTION_ID($2); }
 ...
 */
ctor_initializer.opt:														{ $$ = YACC_MEM_INITIALIZERS(0, 0); }
	|	ctor_initializer
...
mem_initializer_list:
		mem_initializer														{ $$ = YACC_MEM_INITIALIZERS(0, $1); }
...
mem_initializer:
		mem_initializer_id '(' expression_list.opt ')'						{ $$ = YACC_MEM_INITIALIZER($1, $3); }
...
operator_function_id:
		OPERATOR operator													{ $$ = YACC_OPERATOR_FUNCTION_ID($2); }
/*
 ...
 */
operator:
		NEW																	{ add_operator_cost(NEW_OP); $$ = YACC_OPERATOR_NEW_ID(); }
	|	DELETE																{ add_operator_cost(DELETE_OP); $$ = YACC_OPERATOR_DELETE_ID(); }
...
/*  |	DELETE '[' ']'														-- Covered by array of OPERATOR DELETE */
	|	'+'																	{ add_operator_cost(PLUS_OP); $$ = YACC_OPERATOR_ADD_ID(); }
	|	'-'																	{ add_operator_cost(MINUS_OP); $$ = YACC_OPERATOR_SUB_ID(); }
	|	'*'																	{ add_operator_cost(MULT_OP); $$ = YACC_OPERATOR_MUL_ID(); }
	|	'/'																	{ add_operator_cost(DIV_OP); $$ = YACC_OPERATOR_DIV_ID(); }
	|	'%'																	{ add_operator_cost(MOD_OP); $$ = YACC_OPERATOR_MOD_ID(); }
	|	'^'																	{ add_operator_cost(XOR_OP); $$ = YACC_OPERATOR_XOR_ID(); }
	|	'&'																	{ add_operator_cost(AND_OP); $$ = YACC_OPERATOR_BIT_AND_ID(); }
	|	'|'																	{ add_operator_cost(OR_OP); $$ = YACC_OPERATOR_BIT_OR_ID(); }
	|	'~'																	{ add_operator_cost(INV_OP); $$ = YACC_OPERATOR_BIT_NOT_ID(); }
	|	'!'																	{ add_operator_cost(NOT_OP); $$ = YACC_OPERATOR_LOG_NOT_ID(); }
	|	'='																	{ add_operator_cost(ASSIGN_OP); $$ = YACC_OPERATOR_ASS_ID(); }
	|	'<'																	{ add_operator_cost(LT_OP); $$ = YACC_OPERATOR_LT_ID(); }
	|	'>'																	{ add_operator_cost(GT_OP); $$ = YACC_OPERATOR_GT_ID(); }
	|	ASS_ADD																{ add_operator_cost(ASS_ADD_OP); $$ = YACC_OPERATOR_ASS_ADD_ID(); }
	|	ASS_SUB																{ add_operator_cost(ASS_SUB_OP); $$ = YACC_OPERATOR_ASS_SUB_ID(); }
	|	ASS_MUL																{ add_operator_cost(ASS_MUL_OP); $$ = YACC_OPERATOR_ASS_MUL_ID(); }
	|	ASS_DIV																{ add_operator_cost(ASS_DIV_OP); $$ = YACC_OPERATOR_ASS_DIV_ID(); }
	|	ASS_MOD																{ add_operator_cost(ASS_MOD_OP); $$ = YACC_OPERATOR_ASS_MOD_ID(); }
	|	ASS_XOR																{ add_operator_cost(ASS_XOR_OP); $$ = YACC_OPERATOR_ASS_XOR_ID(); }
	|	ASS_AND																{ add_operator_cost(ASS_AND_OP); $$ = YACC_OPERATOR_ASS_BIT_AND_ID(); }
	|	ASS_OR																{ add_operator_cost(ASS_OR_OP); $$ = YACC_OPERATOR_ASS_BIT_OR_ID(); }
	|	SHL																	{ add_operator_cost(SHL_OP); $$ = YACC_OPERATOR_SHL_ID(); }
	|	SHR																	{ add_operator_cost(SHR_OP); $$ = YACC_OPERATOR_SHR_ID(); }
	|	ASS_SHR																{ add_operator_cost(ASS_SHR_OP); $$ = YACC_OPERATOR_ASS_SHR_ID(); }
	|	ASS_SHL																{ add_operator_cost(ASS_SHL_OP); $$ = YACC_OPERATOR_ASS_SHL_ID(); }
	|	EQ																	{ add_operator_cost(EQUAL_OP); $$ = YACC_OPERATOR_EQ_ID(); }
	|	NE																	{ add_operator_cost(NEQ_OP); $$ = YACC_OPERATOR_NE_ID(); }
	|	LE																	{ add_operator_cost(LE_OP); $$ = YACC_OPERATOR_LE_ID(); }
	|	GE																	{ add_operator_cost(GE_OP); $$ = YACC_OPERATOR_GE_ID(); }
	|	LOG_AND																{ add_operator_cost(LOG_AND_OP); $$ = YACC_OPERATOR_LOG_AND_ID(); }
	|	LOG_OR																{ add_operator_cost(LOG_OR_OP); $$ = YACC_OPERATOR_LOG_OR_ID(); }
	|	INC																	{ add_operator_cost(INC_OP); $$ = YACC_OPERATOR_INC_ID(); }
	|	DEC																	{ add_operator_cost(DEC_OP); $$ = YACC_OPERATOR_DEC_ID(); }
	|	','																	{ $$ = YACC_OPERATOR_COMMA_ID(); }
	|	ARROW_STAR															{ add_operator_cost(ARROW_OP);
																			add_operator_cost(STAR_OP);
																			$$ = YACC_OPERATOR_ARROW_STAR_ID(); }
	|	ARROW																{ add_operator_cost(ARROW_OP); $$ = YACC_OPERATOR_ARROW_ID(); }
 ...
 *---------------------------------------------------------------------------------------------------*/
template_declaration:
		template_parameter_clause declaration								{ $$ = YACC_TEMPLATE_DECLARATION($1, $2); }
	|	EXPORT template_declaration											{ $$ = YACC_DECL_SPECIFIER_DECLARATION($2, $1); }

template_parameter_clause:
		TEMPLATE '<' template_parameter_list '>'							{ $$ = $3; }

template_parameter_list:
		template_parameter													{ $$ = YACC_TEMPLATE_PARAMETERS(0, $1); }
	|	template_parameter_list ',' template_parameter						{ $$ = YACC_TEMPLATE_PARAMETERS($1, $3); }

template_parameter:
		simple_type_parameter												{ $$ = YACC_INIT_SIMPLE_TYPE_PARAMETER($1, 0); }
...
simple_type_parameter:
		CLASS																{ $$ = YACC_CLASS_TYPE_PARAMETER(0); }
...
templated_type_parameter:
		template_parameter_clause CLASS										{ $$ = YACC_TEMPLATED_TYPE_PARAMETER($1, 0); }
	|	template_parameter_clause CLASS identifier							{ $$ = YACC_TEMPLATED_TYPE_PARAMETER($1, $3); }

template_id:
		TEMPLATE identifier '<' template_argument_list '>'					{ $$ = YACC_TEMPLATE_NAME($2, $4); }
...
template_argument_list:
		template_argument													{ $$ = YACC_TEMPLATE_ARGUMENTS(0, $1); }
	|	template_argument_list ',' template_argument						{ $$ = YACC_TEMPLATE_ARGUMENTS($1, $3); }

template_argument:
		templated_parameter_declaration										{ $$ = YACC_TEMPLATE_ARGUMENT($1); }
...
explicit_specialization:
		TEMPLATE '<' '>' declaration										{ $$ = YACC_EXPLICIT_SPECIALIZATION($4); }

/*---------------------------------------------------------------------------------------------------
 * A.13 Exception Handling
 *---------------------------------------------------------------------------------------------------*/
try_block:
		TRY compound_statement handler_seq									{ $$ = YACC_TRY_BLOCK($2, $3); }
/*function_try_block:                                                                           -- moved near function_block */

handler_seq:
		handler																{ $$ = YACC_HANDLERS(0, $1); }
	|	handler handler_seq													{ $$ = YACC_HANDLERS($2, $1); }

handler:
		CATCH '(' exception_declaration ')' /*{ put_handler_start(); }*/ compound_statement
																			{ /*put_block_end();*/ $$ = YACC_HANDLER($3, $5); }

exception_declaration:
		parameter_declaration												{ $$ = YACC_EXCEPTION_DECLARATION($1); }
/*                                  ELLIPSIS                                                    -- covered by parameter_declaration */
throw_expression:
		THROW																{ $$ = YACC_THROW_EXPRESSION(0); }
...
exception_specification:
		THROW '(' ')'														{ $$ = YACC_EXCEPTION_SPECIFICATION(0); }
	|	THROW '(' type_id_list ')'											{ $$ = YACC_EXCEPTION_SPECIFICATION($3); }

type_id_list:
		type_id																{ $$ = YACC_EXPRESSIONS(0, $1); }
 ...
 *---------------------------------------------------------------------------------------------------*/
advance_search:
		error																{ yyerrok; advance_search(); } /* Rewind and queue '+' or '-' '#' */
bang:																		{ $$ = YACC_BANG(); } /* set flag to suppress "parse error" */
mark:																		{ $$ = mark(); } /* Push lookahead and input token stream context onto a stack */
nest:																		{ $$ = nest(); } /* Push a declaration nesting depth onto the parse stack */
start_search:																{ $$ = YACC_LINE(); start_search(false); } /* Create/reset binary search context */
start_search1:																{ $$ = YACC_LINE(); start_search(true); } /* Create/reset binary search context */
util:																		{ $$ = YACC_UTILITY_MODE(); } /* Get current utility mode */
