...
extern byte  ***mref;               //!< 1/4 pix luma
extern byte ****mcef;               //!< pix chroma
extern int    **img4Y_tmp;          //!< for quarter pel interpolation
extern byte   **imgY_tmp;           //!< for loop filter
extern byte  ***imgUV_tmp;
...
extern byte  **mref_P;              //!< for B-frames: 1/4 pix luma for next P picture
extern byte ***mcef_P;              //!< for B-frames: pix chroma for next P picture
extern byte  **nextP_imgY;
extern byte ***nextP_imgUV;
extern pel_t **Refbuf11;            //!< 1/1th pel (full pel) reference frame buffer
extern pel_t  *Refbuf11_P;          //!< 1/1th pel P picture buffer

// Buffers for rd optimization with packet losses, Dim. Kontopodis
extern int  **resY;             //!< Residue of Luminance
extern byte ***decY;            //!< Decoded values at the simulated decoders
extern byte ****decref;         //!< Reference frames of the simulated decoders
extern byte ***decY_best;       //!< Decoded frames for the best mode for all decoders
extern byte **RefBlock;
extern byte **status_map;
extern byte **dec_mb_mode;
extern byte **dec_mb_ref;
extern int intras;       //!< Counts the intra updates in each frame.

extern int  Bframe_ctr, frame_no, nextP_tr;
extern int  tot_time;
