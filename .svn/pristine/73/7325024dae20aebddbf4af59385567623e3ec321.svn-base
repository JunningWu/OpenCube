...
// bool ExpatXMLParser::setBlockSize (size_t size) {
//   if (xml_buffer)
//       delete xml_buffer;
//   xml_buffer_size = size;
//   xml_buffer = new XML_Char[size];
//   if (xml_buffer == NULL) {
//       return false;
//   }
//   memset (xml_buffer, 0, size);
//   return true;
// }
...
ExpatXMLParser::StartElement(const XML_Char *name, const XML_Char **atts) {
  //printf("[XML]Start:\t%s\n", name);
  std::cout << "[XML]Start:\t" << name << std::endl;
}

void
ExpatXMLParser::EndElement(const XML_Char *name) {
  //printf("[XML]End:\t%s\n", name);
  std::cout << "[XML]End:\t" << name << std::endl;
}
